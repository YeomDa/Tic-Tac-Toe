    ...
    def __init__(self, user, title, isHost,mode):
        ...
        if doc.exists:
            if(self.mode == 'network_3'):
                ...
                data_list = doc.to_dict()
                self.play_omokgame_count = data_list.get('play_omokgame_count') #경로의 데이터를 list화 시키고, 'play_game_count'의 값을 가져옵니다.
        ...
        if(self.mode =='network_3'):    
            ...
            if(self.play_omokgame_count == None) : self.play_omokgame_count = 0
